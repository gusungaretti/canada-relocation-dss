...
export default function CityRankingList({
  ...
  // Shortlist mode groups the list into the k chosen cities and the rest, but only in the
  // default GP order — an explicit column sort takes precedence and shows a flat table.
  const hasShortlist = cities.some((c) => c.inShortlist)
  const grouped = hasShortlist && !effectiveSortKey
      ...
      >
        ...
        <td className="px-2 py-2.5">
          ...
          >
            ...
            {city.parentName && (
              <span className="ml-1.5 text-[11px] text-neutral-300">
                · suburb of {city.parentName}
              </span>
            )}
            {inShortlist && (
              <span className="ml-1.5 text-[9px] font-mono uppercase tracking-wider px-1.5 py-0.5 border border-emerald-600/40 text-emerald-700">
                Shortlist
              </span>
            )}
          </button>
  ...
  function renderGroupHeader(label: string) {
    const colSpan = 3 + activeFactors.length + 1
    return (
      <tr key={`header-${label}`} className="border-b border-black/[0.05] bg-neutral-50/80">
  ...
  const shortlistCities = grouped ? displayedCities.filter((c) => c.inShortlist) : []
  const otherCities = grouped ? displayedCities.filter((c) => !c.inShortlist) : []
      ...
      <table className="w-full min-w-max border-collapse">
        ...
        <tbody>
          {grouped ? (
            ...
            displayedCities.map((city, i) => renderRow(city, i))
          )}
        </tbody>
      </table>
    </div>
